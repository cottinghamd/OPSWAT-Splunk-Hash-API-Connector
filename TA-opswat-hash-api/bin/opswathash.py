...
import ta_opswat_hash_api_declare

import os
import sys

from alert_actions_base import ModularAlertBase
import modalert_opswathash_helper

class AlertActionWorkeropswathash(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkeropswathash, self).__init__(ta_name, alert_name)
    ...
    def validate_params(self):
        ...
        if not self.get_global_setting("host"):
            self.log_error('host is a mandatory setup parameter, but its value is None.')
            return False
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_opswathash_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkeropswathash("TA-opswat-hash-api", "opswathash").run(sys.argv)
    sys.exit(exitcode)
